...
export function MindmapSidebarDaySection({
  day,
  courseCode,
  activeFileId,
  defaultOpen,
      ...
      {day.files.length > 0 ? (
        <div className="flex flex-col gap-2 px-4 pb-3.5">
          {day.files.map((file) => {
            ...
            const hasMindmap = Boolean(MINDMAPS[file.id])
            return (
              <Link
                key={file.id}
              ...
              >
                <Workflow className="size-4 shrink-0 text-navy" />
                <span className="min-w-0 flex-1">
                  <span className="block truncate text-[13px] font-semibold text-ink">
                    {file.fileName}
                  </span>
                  <span className="font-mono text-[11px] text-ink-soft">
                    {hasMindmap ? "Sơ đồ tư duy" : "Chưa có sơ đồ"}
                  </span>
                </span>
                {active && (
